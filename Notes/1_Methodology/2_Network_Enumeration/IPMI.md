...
```
###### Set the target IP address to extract hashes from.      
```
set rhosts 10.129.42.195
```
###### Execute the module to retrieve the password hashes (and automatically attempts to crack common ones if the `CRACK_COMMON` option is left to true).        
```
run
```
...
###### Uses Hashcat in mask attack mode (`-a 3`) to crack the obtained IPMI hashes (hash mode `7300`). This specific mask (`?1?1?1?1?1?1?1?1` where `?1` is `?d?u`) is highly effective for cracking factory default HP iLO passwords, which consist of 8 randomized numbers and uppercase letters.
